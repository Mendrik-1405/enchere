...
CREATE TABLE enchere(
    ...
    etat int not null,--0 en cours 1 termine
...
$$
    ...
    BEGIN
        ...
        select coalesce(pourcentage,0) from commission into pourcent;
...
Declare
    ...
    prixminimum double precision;
    prixtemp double precision;
BEGIN
    IF (select etat from enchere where id = new.enchereid) = 50 then
        RAISE EXCEPTION '10#Enchere deja cloture';
    END IF;
    IF (select solde from soldelogin where id = new.loginid)  < new.prix then
        RAISE EXCEPTION '10#Vous n''avez pas assez d''argent pour sur encherir';
    END IF;
    select * into lastrows from mise where enchereid=New.enchereid;
    select prixminmise into prixminimum from enchere where id=new.enchereid;
    select prixmin into prixtemp from enchere where id=new.enchereid;
    ...
    END IF;
    IF lastrows.prix > 0 then
        prixtemp := lastrows.prix;
    ELSE 
        prixminimum := 0;
    END IF;
    IF (prixtemp+prixminimum) > new.prix and new.loginid != 0 then
        RAISE EXCEPTION '30#La mise doit etre supperieur à la derniere mise qui s''eleve à % avec mise minimum de %',prixtemp,(prixtemp+prixminimum);
    END if;
...
create or replace FUNCTION addBeforeEnchere() 
RETURNS TRIGGER as $$
BEGIN
    IF (select count(*) from produit where id = new.produitid and loginid = new.loginid) = 0 then
        RAISE EXCEPTION 'EPr1#Le produit n''appartient pas au proprietaire de l''enchere';
    end if;
END;
$$
LANGUAGE plpgsql;

CREATE TRIGGER addBeforeEnchere
BEFORE INSERT ON enchere
FOR EACH ROW EXECUTE FUNCTION addBeforeEnchere();
...
CREATE OR REPLACE view vtauxremise as
    SELECT 
        ca.id, 
        AVG((c.montant - e.prixmin) / e.prixmin * 100) as taux_remise
    FROM 
        comptesite c
            left join enchere e 
            on c.enchereid = e.id
...
insert into login (id, etat) values(0,50);
...
insert into categorie(designation) values('chaussure');
insert into categorie(designation) values('Vetement');

insert into produit(designation,loginid,categorieid,descri,prixmin,prixmax) values
('Produit1',1,1,'Une petite description pour le produit1',100,300),
('Produit2',1,2,'Une petite description pour le produit2',200,300),
('Produit3',1,1,'Une petite description pour le produit3',300,300),
('Produit4',2,1,'Une petite description pour le produit4',400,300),
('Produit5',2,1,'Une petite description pour le produit5',500,300)
;
